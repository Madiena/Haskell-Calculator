module Service.AbstractSyntax (Expression (..), OpCode (..), Definition (..)) where
...
-}
data Definition
  = FunctionDef {funcName :: String, funcParams :: [String], funcBody :: Expression}
  | VariableDef {varName :: String, expr :: Expression}
  deriving (Show, Eq)
